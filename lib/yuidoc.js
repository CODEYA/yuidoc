...
YUI.add('yuidoc', function (Y) {
    ...
    Y.YUIDoc = function (config) {
        ...
        if (this.options.addondir) {
            if (path.isAbsolute(this.options.addondir)) {
                var dir = this.options.addondir;
            } else {
                var dir = path.join(process.cwd(), this.options.addondir);
            }
            Y.log("addon dir: " + dir);

            fs.readdirSync(dir).forEach(function (file) {
                Y.log("loading addon: " + file);
                var addon = require(dir + '/' + file);
                addon(Y);
            });
        }
